...
export function signAccess(userId: bigint): string {
...
export function verifyAccess(token: string): AccessPayload {
  return jwt.verify(token, env.JWT_SECRET, { issuer: 'bacapi' }) as AccessPayload
}

// ---------- refresh tokens (opaque, rotating) ----------

export async function issueRefresh(userId: bigint, familyId?: string): Promise<string> {
...
export type RotateResult = { userId: bigint; access: string; refresh: string }
 ...
 */
export async function rotateRefresh(token: string): Promise<RotateResult | null> {
...
export async function revokeAllForUser(userId: bigint) {
  await prisma.refresh_tokens.updateMany({
 ...
 */
export async function issueAuthToken(userId: bigint, purpose: Purpose): Promise<string> {
