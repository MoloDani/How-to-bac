...
import { registerBodyDto, loginBodyDto, refreshBodyDto, tokenBodyDto, forgotBodyDto, resetBodyDto, } from './dto.js';
import { registerSchema, loginSchema, refreshSchema, logoutSchema, verifyEmailSchema, resendVerificationSchema, forgotSchema, resetSchema, meSchema, } from './swagger.js';
import { signAccess, issueRefresh, rotateRefresh, revokeByToken, revokeAllForUser, issueAuthToken, consumeAuthToken, } from './tokens.js';
...
async function makeHandle(addr) {
    const base = addr.split('@')[0].toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 20) || 'user';
    for (let i = 0; i < 5; i++) {
...
const authRoutes = async (app) => {
    ...
    app.post('/logout', { schema: logoutSchema }, async (req, reply) => {
        const parsed = refreshBodyDto.safeParse(req.body);
        if (parsed.success)
            await revokeByToken(parsed.data.refresh);
    ...
    app.post('/forgot', { ...strict, schema: forgotSchema }, async (req, reply) => {
        ...
        if (user?.email) {
            await sendPasswordResetEmail(user.email, await issueAuthToken(user.id, 'password_reset'));
        }
        return reply.send({ ok: true });
    });
    // ── reset password ──────────────────────────────────────────────
